...
local function recursiveCopyTable(oldTable,oldString,newString)
	...
	for k,d in pairs(oldTable) do
		...
		elseif type(d)=="table" then
			newTable[k]=recursiveCopyTable(d,oldString,newString,fh) 
		elseif type(d)=="string" and k~="type" then
			if string.find(newString,"hybrid") then
	end
	newTable[k]=d
			if string.ends(newTable[k],"/"..oldString..".png") then
				newTable[k]=string.gsub(newTable[k],"__.-__/(.+)","__"..ModName.."__/%1")
			end
			if string.find(newTable[k],"__"..ModName.."__") or not string.find(newTable[k],"__.-__") then
...
createData=function(objectType,original,newName,newData)
	...
	newEntity=recursiveCopyTable(newEntity,original,newName,fh)
	if newData then
